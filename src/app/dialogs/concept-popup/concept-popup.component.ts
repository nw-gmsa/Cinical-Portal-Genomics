...
export class ConceptPopupComponent implements OnInit {
  readonly data = inject<Coding>(MAT_DIALOG_DATA);
  ...
  ngOnInit(): void {
    if (this.data.code !== undefined && this.data.system !== undefined) {
      this.http.get(this.validateUrl + '/CodeSystem/\$lookup?system='+this.data.system
    ...
    if (this.data.code !== undefined && this.data.system !== undefined  && this.data.display !== undefined) {
      this.http.get(this.validateUrl + '/ValueSet/\$validate-code?system='+this.data.system
          + '&code='+ this.data.code
